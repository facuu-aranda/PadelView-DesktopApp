    ...
    try {
      ...
      for (const match of matches as Match[]) {
        ...
        if (isTimeForRecording && !ffmpegService.isRecording(match.id)) {
          ...
          console.warn(`Match ${match.id} end time passed but was never recorded. Marking as FAILED.`);
          await db
            .from('matches')
            .update({
              status: 'FAILED',
    ...
    for (const match of matches as Match[]) {
      ...
      if (match.status === 'RECORDING') {
        ...
        if (fs.existsSync(filePath)) {
          ...
          if (stats.size > 1024 * 1024) { // > 1MB partial video
            console.log(`Recovered partial recording file for match ${match.id}. Queuing for upload.`);
            // Queue for upload
            this.processUploadStart(match, filePath).catch(err => {
              console.error(`Error recovering upload for match ${match.id}:`, err);
            });
          } else {
            console.log(`Found empty or corrupt recording file for match ${match.id}. Marking as FAILED.`);
            await db
              .from('matches')
              .update({ status: 'FAILED', error_log: 'Recording interrupted (power outage or crash) and file is empty.' })
              .eq('id', match.id);
            // Cleanup empty file
            fs.unlink(filePath, () => {});
          }
        } else {
          console.log(`No recording file found for interrupted match ${match.id}. Marking as FAILED.`);
          await db
            .from('matches')
        ...
        if (fs.existsSync(filePath)) {
          ...
          await db
            .from('matches')
            .update({ status: 'FAILED', error_log: 'Upload interrupted and local video file was deleted.' })
            .eq('id', match.id);
        }
      }
    }
