...
const DEFAULT_CUSTOM_THEMES: CustomTheme[] = [
  { bg: '#0f172a', card: 'rgba(30, 41, 59, 0.7)', text: '#f8fafc', primary: '#3b82f6', sidebar: '#0f172a' },
  { bg: '#18181b', card: 'rgba(39, 39, 42, 0.7)', text: '#f4f4f5', primary: '#ec4899', sidebar: '#18181b' },
  { bg: '#120f26', card: 'rgba(25, 20, 48, 0.7)', text: '#f3f4f6', primary: '#8b5cf6', sidebar: '#120f26' }
];
...
function LiveCourtStream({ courtId, rtspUrl }: LiveCourtStreamProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    console.log(`[Renderer] LiveCourtStream useEffect mounted for court ${courtId}. JSMpeg exists:`, !!JSMpeg, "Canvas exists:", !!canvasRef.current);
    ...
    const handleData = (_event: any, chunk: Uint8Array) => {
      ...
      if (chunkCount <= 5 || chunkCount % 100 === 0) {
        console.log(`[Renderer] Received chunk #${chunkCount} for court ${courtId} (size: ${chunk.length})`);
      }
  ...
  if (!rtspUrl) {
    return (
      <div style={{ position: 'relative', width: '100%', height: '180px', backgroundColor: '#000', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '180px', backgroundColor: '#000', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <span className="badge-live animate-pulse-fast" style={{ position: 'absolute', top: '10px', left: '10px', zIndex: 10, fontSize: '10px', background: '#ef4444', color: '#fff', padding: '2px 6px', borderRadius: '4px', fontWeight: 'bold' }}>
        EN VIVO
      </span>
    </div>
  );
...
function App(): React.JSX.Element {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'scheduler' | 'courts' | 'config' | 'profile'>('dashboard');
  ...
  const handleToggleStartup = async (enabled: boolean) => {
    ...
    if (res.success) {
      ...
      showToast(enabled ? 'Inicio automático activado.' : 'Inicio automático desactivado.', 'success');
    } else {
  ...
  return (
    <div className={`app-container ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      <aside className="sidebar">
        <div className="brand" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '0 4px', marginBottom: '36px' }}>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {sidebarCollapsed ? (
              <button className="brand-logo-btn" onClick={handleToggleSidebar} title="Desplegar menú">
                <span className="logo-icon-text">PV</span>
                <span className="logo-chevron-icon">
                  <ChevronRight size={20} />
                </span>
              </button>
            ) : (
              <div className="brand-logo-static">PV</div>
            )}
            
            <div className="brand-text-container">
              <h2 className="brand-title" style={{ margin: 0, fontSize: '16px', fontWeight: 'bold' }}>PadelView</h2>
              <span className="badge" style={{ fontSize: '10px', marginTop: '2px', marginLeft: 0 }}>MVP</span>
            </div>
          </div>

          {!sidebarCollapsed && (
            <button className="sidebar-collapse-btn-right" onClick={handleToggleSidebar} title="Colapsar menú">
              <ChevronLeft size={16} />
            </button>
          )}
        </div>
      ...
      <main className="main-content">
        ...
        <header className="top-header">
          <div className="header-title">
            ...
            <p className="text-secondary">{clubName} - {loginOperator}</p>
          </div>
        ...
        {!ffmpegStatus.exists && (
          <div className="banner danger animate-pulse">
            ...
            <div className="banner-text">
              <strong>Error de FFmpeg:</strong> No se detectó el binario de FFmpeg en el sistema (Ruta: {ffmpegStatus.path}). Las grabaciones de vídeo no funcionarán.
            </div>
          </div>
        )}
                ...
                <div className="courts-status-section">
                  ...
                  {courts.length === 0 ? (
                    <div className="empty-state">
                      ...
                      <button className="btn btn-primary btn-sm" onClick={() => setActiveTab('courts')}>
                    ...
                    <div className="court-cards">
                      {courts.map(court => {
                        ...
                        const isPreviewActive = courtPreviewEnabled[court.id] !== undefined
                          ...
                          <div key={court.id} className={`court-card ${isRecording ? 'recording' : ''}`}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <div className="card-options-container">
                                  <button 
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setOpenDropdownCourtId(openDropdownCourtId === court.id ? null : court.id);
                                    }}
                                  >
                                    <MoreVertical size={14} />
                                  </button>
                                  {openDropdownCourtId === court.id && (
                                    <div className="card-options-dropdown" onClick={(e) => e.stopPropagation()}>
                                      <button
                                        type="button"
                            ...
                            <div className="court-card-body">
                              ...
                              {isRecording && activeMatch ? (
                                ...
                                <div className={isPreviewActive ? "idle-state-condensed" : "idle-state"}>
                          ...
                          <tr key={match.id}>
                            ...
                            <td>
                              <span className={`status-pill ${match.status.toLowerCase()}`}>
                                {match.status === 'DONE' ? 'LISTO' : 
                                 match.status === 'RECORDING' ? 'GRABANDO' : 
                                 match.status === 'UPLOADING' ? 'SUBIENDO' : 
                                 match.status === 'FAILED' ? 'FALLIDO' : 'AGENDADO'}
                              </span>
                            </td>
                            <td>
          ...
          {activeTab === 'courts' && (
            <div className="courts-view">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h3 className="section-title" style={{ margin: 0 }}>Canchas</h3>
                <button 
              ...
              {courts.length === 0 ? (
                <div className="empty-state">
                  ...
                  <button className="btn btn-primary btn-sm" onClick={() => setIsCreateCourtModalOpen(true)}>
                ...
                <div className="courts-grid">
                  {courts.map(court => {
                    ...
                    return (
                      <div key={court.id} className={`court-bento-card ${isRecording ? 'recording' : ''}`}>
                        <div className="bento-card-header">
                          <h3>{court.name}</h3>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <span className={`bento-status-badge ${status}`}>
                              {status === 'grabando' ? 'Grabando' : 
                               status === 'reservada' ? 'Reservada' : 'Libre'}
                            </span>
              ...
              {isCreateCourtModalOpen && (
                <div className="modal-overlay" onClick={() => setIsCreateCourtModalOpen(false)}>
                  <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                    ...
                    <h3 className="card-title" style={{ marginBottom: '20px' }}>Registrar Nueva Cancha</h3>
            ...
            <div className="config-view">
              <div className="config-form form-grid">
                  ...
                  <div className="theme-customizer">
                    ...
                    {themePreset.startsWith('custom') && (
                      <div className="custom-theme-editor animate-pulse-fast">
                        <h4>Diseñando {themePreset === 'custom1' ? 'Tema 1' : themePreset === 'custom2' ? 'Tema 2' : 'Tema 3'}</h4>
                              ...
                              <>
                                <div className="color-picker-item">
                                  ...
                                  <input
                                    type="color"
                                    value={t.bg}
                                    onChange={e => handleCustomColorChange(idx, 'bg', e.target.value)}
                                  />
                                </div>
                                <div className="color-picker-item">
                                  ...
                                  <input
                                    type="color"
                                    value={t.card}
                                    onChange={e => handleCustomColorChange(idx, 'card', e.target.value)}
                                  />
                                </div>
                                <div className="color-picker-item">
                                  ...
                                  <input
                                    type="color"
                                    value={t.text}
                                    onChange={e => handleCustomColorChange(idx, 'text', e.target.value)}
                                  />
                                </div>
                                <div className="color-picker-item">
                                  ...
                                  <input
                                    type="color"
                                    value={t.primary}
                                    onChange={e => handleCustomColorChange(idx, 'primary', e.target.value)}
                                  />
                                </div>
                                <div className="color-picker-item" style={{ gridColumn: '1 / -1' }}>
                                  ...
                                  <input
                                    type="color"
                                    value={t.sidebar}
                                    onChange={e => handleCustomColorChange(idx, 'sidebar', e.target.value)}
                                  />
                                </div>
                              </>
                            );
                ...
                <div className="card-pane">
                  ...
                  <div className="form-group" style={{ marginBottom: '16px' }}>
                    <label className="login-checkbox-group" style={{ color: 'var(--color-text-primary)' }}>
                      <input
                        type="checkbox"
                        ...
                        onChange={async (e) => {
                          ...
                          showToast(val ? 'Notificaciones activadas.' : 'Notificaciones desactivadas.', 'info');
                        }}
                      />
                    ...
                    <label className="login-checkbox-group" style={{ color: 'var(--color-text-primary)' }}>
                      <input
                        ...
                        onChange={e => {
                          ...
                          showToast(val ? 'Previsualización por defecto activada.' : 'Previsualización por defecto desactivada.', 'info');
                        }}
                      />
                  ...
                  {courts.length > 0 && (
                    <div style={{ marginTop: '24px', borderTop: '1px solid var(--color-border)', paddingTop: '16px' }}>
                      <h4 style={{ fontSize: '14px', marginBottom: '10px' }}>Enlaces de Transmisión RTSP (Cámaras)</h4>
                      {courts.map(c => (
            ...
            <div className="profile-view-wrapper">
              <form onSubmit={handleSaveProfile} className="profile-view">
                <div className="profile-avatar-card">
                  ...
                  <h3 style={{ fontSize: '18px', fontWeight: '700', marginTop: '10px' }}>{profileName}</h3>
                ...
                <div className="subscription-section">
                  ...
                  {subType === 'lifetime' && (
                    <div className="sub-card sub-vitalicia">
                      ...
                      <p>¡Muchas gracias por apoyar el proyecto PadelView! Tu club tiene todos los privilegios desbloqueados de forma indefinida.</p>
                  ...
                  {subType === 'active' && (
                    <div className="sub-card sub-activa">
                      ...
                      <p>Suscripción registrada correctamente. Tu licencia expira el: <strong>{subExpiry}</strong>.</p>
                  ...
                  {subType === 'inactive' && (
                    <div className="sub-card sub-inactiva">
                      ...
                      <p>No tienes ninguna suscripción activa para este club. Los videos no se procesarán ni subirán a la nube.</p>
                      <button
                        type="button"
                        className="btn btn-primary btn-sm"
